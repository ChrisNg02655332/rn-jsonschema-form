...
const ROOT_SCHEMA_PREFIX = '__rnjsf_rootSchema'
 ...
 *};
 */
const toErrorSchema = (errors: Array<Error>) => {
  if (!errors.length) {
    return {}
  }
...
export const toErrorList = (errorSchema: any, fieldName = 'root') => {
  // XXX: We should transform fieldName as a full field path string.
  let errorList: any = []
  if ('__errors' in errorSchema) {
    errorList = errorList.concat(
      errorSchema.__errors.map((stack: any) => {
        return {
...
const createErrorHandler = (formData: any): any => {
  const handler = {
    // We store the list of errors for this node in a property named __errors
    // to avoid name collision with a possible sub schema field named
...
const unwrapErrorHandler = (errorHandler: any): any => {
...
export default function validateFormData(
  ...
  if (customFormats && newFormats && isObject(customFormats)) {
    Object.keys(customFormats).forEach((formatName) => {
      ajv.addFormat(formatName, (customFormats as any)[formatName])
    })
  ...
  let errors = transformAjvErrors(ajv.errors as any)
 ...
 */
export function withIdRefPrefix(schemaNode: any) {
  let obj = schemaNode
 ...
 * false.
 */
export const isValid = (schema: any, data: any, rootSchema: any) => {
  try {
